export function generatePolicy(formData) {
  const {
    appName,
    companyName,
    contactEmail,
...
${appName || '本应用'} 是一款由 ${companyName || '我们公司'} （以下简称"我们"）提供的产品。您在使用我们的服务时，我们可能会收集和使用您的相关信息。我们希望通过本《隐私政策》向您说明，在使用我们的服务时，我们如何收集、使用、储存和分享这些信息，以及我们为您提供的访问、更新、控制和保护这些信息的方式。

本《隐私政策》与您所使用的 ${appName || '本应用'} 服务息息相关，希望您仔细阅读，在需要时，按照本《隐私政策》的指引，作出您认为适当的选择。本《隐私政策》中涉及的相关技术词汇，我们尽量以简明扼要的表述，并提供进一步说明的链接，以便您的理解。

**您使用或继续使用我们的服务，即意味着同意我们按照本《隐私政策》收集、使用、储存和分享您的相关信息。**

如对本《隐私政策》或相关事宜有任何问题，请通过 **${contactEmail || 'contact@example.com'}** 与我们联系。
...
`
        if (sdk.company) policy += `- **第三方名称**：${sdk.company}\n`
        if (sdk.purpose) policy += `- **使用目的**：${sdk.purpose}\n`
        if (sdk.dataTypes) policy += `- **收集个人信息类型**：${sdk.dataTypes}\n`
        if (sdk.processing) policy += `- **处理方式**：${sdk.processing}\n`
        if (sdk.privacyUrl) policy += `- **隐私政策链接**：${sdk.privacyUrl}\n`
        policy += `\n`
      }
    })
  }
...
- **公司名称**：${companyName || '我们公司'}
- **联系邮箱**：${contactEmail || 'contact@example.com'}
`
